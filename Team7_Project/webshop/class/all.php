...
Class all {
  ...
  public function __construct(){
    ...
    $this->setCookie();
    // Session Timeout after 120 seconds of system inactivity
    if(isset($_SESSION['INACTIVE']) && $_SESSION['INACTIVE']+120 <= time() ){
      session_destroy();
    }
    // getting the current time into the session 
...
// method used to add cookie to database
 ...
 // list down all products from products table
...
public function login(){
  ...
  if(empty($user) || empty($pass)) {
      ...
      $sql = "SELECT id, firstname, email, address, password FROM users WHERE email=? ";
      $query = $this->_conn->prepare($sql);
      $query->execute(array($user));
      //  $query->debugDumpParams();
      $fetch =$query->fetchAll();
      $fetchedPass = $fetch[0]['password'];
      // verifying password after fetching from database
      if(password_verify($pass,$fetchedPass) && $query->rowCount() >= 1)
      {
        $_SESSION['user'] = $user;
          $_SESSION['userid'] = $fetch[0]['id'];
          $_SESSION['details'] =$fetch[0];
...
// User Register function, user is added to users table
public function Userregister()
{
  ...
  // hashing the password using salting
...
// function to list all the items from table basket
...
// function to remove  a single item from basket
  ...
  // function to show user profile details fetched from users table
...
// function to get my orders from orders table
